...
import {
  fetchAdminPersonalTransactions,
  TransactionsApiError,
} from "./lib/transactions-api";
import {
  fetchAdminPersonalSummary,
  SummaryApiError,
} from "./lib/summary-api";
import {
  fetchAdminPersonalInvestments,
  InvestmentsApiError,
...
type AdminMyAccountDashboardPageProps = {
...
export default async function AdminMyAccountDashboardPage({
  searchParams,
}: AdminMyAccountDashboardPageProps) {
  ...
  const [summaryResult, transactionsResult, investmentsResult] =
    await Promise.allSettled([
      fetchAdminPersonalSummary(),
      fetchAdminPersonalTransactions(currentPage),
      fetchAdminPersonalInvestments(),
    ]);
  ...
  function pageHref(pageNumber: number): string {
    if (pageNumber <= 1) return "/admin/my-account/dashboard";
    return `/admin/my-account/dashboard?page=${pageNumber}`;
  }
